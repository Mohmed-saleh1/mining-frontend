...
export default function AdminDashboardPage() {
  ...
  const stats = [
    {
      label: t('stats.totalMachines'),
      value: machines?.length || 0,
      icon: "M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01",
      color: "gold",
    },
    {
      label: t('stats.activeMachines'),
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <div className="glass rounded-xl p-6 animate-fade-in-up stagger-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <p className="text-sm font-medium text-foreground">{t('quickActions.manageMachines')}</p>
            </Link>
            <Link
            ...
            >
              ...
              <p className="text-sm font-medium text-foreground">{t('quickActions.addNewMachine')}</p>
            </Link>
            <Link
              href="/admin/users"
              className="p-4 rounded-lg bg-gold/10 border border-gold/20 hover:border-gold/40 transition-all group"
            >
              ...
              <p className="text-sm font-medium text-foreground">{t('quickActions.manageUsers')}</p>
            </Link>
            <Link
            ...
            >
              ...
              <p className="text-sm font-medium text-foreground">{t('quickActions.legalDocuments')}</p>
            </Link>
            <Link
              href="/"
              className="p-4 rounded-lg bg-gold/10 border border-gold/20 hover:border-gold/40 transition-all group"
            >
              ...
              <p className="text-sm font-medium text-foreground">{t('quickActions.viewSite')}</p>
            </Link>
          </div>
        </div>
        ...
        <div className="glass rounded-xl p-6 animate-fade-in-up stagger-3">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-foreground">{t('recentMachines.title')}</h2>
            <Link href="/admin/machines" className="text-sm text-gold hover:underline">
              {t('recentMachines.viewAll')}
            </Link>
          </div>
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-foreground-muted text-sm">{t('recentMachines.noMachines')}</p>
              <Link href="/admin/machines?action=new" className="text-gold text-sm hover:underline">
                {t('recentMachines.addFirstMachine')}
              </Link>
            </div>
          ) : (
            <div className="space-y-3">
